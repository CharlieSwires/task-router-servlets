...
class CreateWorkspaceTask {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            addActivitiesToWorkspace(workspaceFacade, workspaceConfig);
    ...
    public static void addActivitiesToWorkspace(WorkspaceFacade workspaceFacade,
            JsonObject workspaceJsonConfig) {
        JsonArray activitiesJson = workspaceJsonConfig.getJsonArray("activities");

        activitiesJson.getValuesAs(JsonObject.class).forEach(activityJson -> {
            Map<String, String> activityParams = new HashMap<>();
            activityParams.put("ActivityName", activityJson.getString("name"));
            activityParams.put("Availability",
                    activityJson.getString("availability"));

            try {
                workspaceFacade.addActivity(activityParams);
            } catch (TaskRouterException e) {
                LOG.warning(e.getMessage());
            }
        });
    ...
    public static void addTaskQueuesToWorkspace(WorkspaceFacade workspaceFacade,
        ...
        Activity reservationActivity = workspaceFacade.findActivityByName("Reserved")
                .orElseThrow(() ->
        new TaskRouterException("The activity for reservations 'Reserved' was not found. "
                + "TaskQueues cannot be added."));
        Activity assignmentActivity = workspaceFacade.findActivityByName("Busy")
                .orElseThrow(() ->
    ...
    public static Workflow addWorkflowToWorkspace(WorkspaceFacade workspaceFacade,
        ...
        return workspaceFacade.findWorkflowByName(workflowName)
                .orElseGet(() -> {
                    ...
                    workflowParams.put("AssignmentCallbackUrl",
                            workflowJson.getString("callback"));
                    workflowParams.put("FallbackAssignmentCallbackUrl",
                            workflowJson.getString("callback"));
                    workflowParams.put("TaskReservationTimeout",
                            workflowJson.getString("timeout"));

                    String workflowConfigJson = createWorkFlowJsonConfig(
                            workspaceFacade, workflowJson);
    ...
    public static void printSuccessAndExportVariables(WorkspaceFacade workspaceFacade,
        ...
        workspaceParams.entrySet().stream().forEach(propertyEntry -> {
            System.out.println(String.format("%s=%s", propertyEntry.getKey(),
    ...
    public static JsonObject createWorkspaceConfig(String[] args) {
        ...
        System.out.println("url1 : " + configFileName);
        System.out.println("url3 : " + "/" + configFileName);
        Optional<URL> url =
                Optional.ofNullable(CreateWorkspaceTask.class.getResource("/" + configFileName));
        if (url.isPresent()) {
            System.out.println("url2 : " + url.get());
        }
        return url.map(u -> {
            try {
                ...
                () -> new TaskRouterException("There's no valid configuration in "
        + configFileName));
    }
    ...
    public static String createWorkFlowJsonConfig(WorkspaceFacade workspaceFacade,
        ...
        try {
            ...
            WorkflowRuleTarget defaultRuleTarget = new WorkflowRuleTarget.Builder(
                    defaultQueue.getSid())
                    .expression("1=1")
                    .priority(1)
                    .timeout(1800)
            ...
            List<WorkflowRule> rules = routingConfigRules.getValuesAs(JsonObject.class).stream()
                    .map(ruleJson -> {
                        ...
                        TaskQueue ruleQueue = workspaceFacade
                                .findTaskQueueByName(ruleQueueName).orElseThrow(
                                () -> new TaskRouterException(String
                                        .format("%s queue not found", ruleQueueName)));
                        ...
                        List<WorkflowRuleTarget> ruleTargets = Arrays.asList(
                                queueRuleTarget, defaultRuleTarget);

                        return new WorkflowRule.Builder(ruleJson
                                .getString("expression"), ruleTargets).build();
